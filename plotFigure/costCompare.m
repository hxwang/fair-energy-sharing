...
CGcostfile = '\CGAssigner_Meancost.txt';
NEScostfile = '\NoShareAssigner_Meancost.txt';

alpha = 0.1


for i = 1:1:len
    CGfileName = strcat(fileDir, num2str(i*10), CGcostfile);
    NESfileName = strcat(fileDir, num2str(i*10), NEScostfile);
...
set(get(axes1,'XLabel'),'String','HOC ratio','FontSize',30,'FontWeight','bold');
...
saveas(gcf, '.\figures\costCompare', 'pdf') %Save figure  
saveas(gca, strcat('.\figures\costCompare', '.eps'),'psc2') %Save figure 
