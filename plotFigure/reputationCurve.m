function reputationCurve( assigner, ratio)
...
fileDir =  '..\simulationResult\ReputationCurve\';


fileName = strcat(fileDir, num2str(ratio*100),'\',assigner,'_reputationList.txt');
data = load(fileName);


EnergyfileName = strcat(fileDir, num2str(ratio*100),'\',assigner,'_energyIn.txt');
energydata = load(EnergyfileName);

[maxVal, idx] = max(energydata)
...
[AX,H1,H2] = plotyy(x(1,:),data(:,1),x(1,:),energydata(1,:),'plot');
   
set(AX(1),'XColor','k','YColor','k','ytick',[0:50:200],'FontSize',26,'FontWeight','bold');
set(AX(2),'XColor','k','YColor','b','ytick',[0:25:50],'FontSize',26','FontWeight','bold');


set(H1,'Color','g','LineWidth',2,'linestyle','-')
set(H1,'Marker','o');   
set(H2,'Color','b','LineWidth',2,'linestyle','--')
set(H2,'Marker','x','Markersize',15);


% p= plot(x(1,:),data(:,1))
%  set(p,'Color','r','LineWidth',2,'linestyle','-')
% %set(p,'Color','b')
% set(p,'Marker','>','Markersize',5);
% 
% p= plot(x(1,:), data(:,1)-data(:,2))
%  set(p,'Color','b','LineWidth',2,'linestyle','-')
% %set(p,'Color','b')
...
legend(axes1,'show');
leg = legend('Reputation', 'Assigned energy')
...
legend([H1 H2],'Reputation','Energe acquisition','Location','NorthWest');
...
saveas(gcf, strcat('.\figures\reputationCurve', assignerLabel, '_', num2str(ratio*100)), 'pdf') %Save figure  
saveas(gca, strcat('.\figures\reputationCurve',assignerLabel, '_', num2str(ratio*100), '.eps'),'psc2') %Save figure 
